...
export async function requestParagraphRewrite(params:{paragraph:string;prompt?:string;module:'academic'|'general'}):Promise<RewriteResult>{const res=await fetch('/api/writing/improve',sameOriginInit({method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(params)}));if(!res.ok){const err=await res.json().catch(()=>({}));throw new GradingError(String(err.code||'unknown'),String(err.error||'Rewrite failed'),err);}return res.json();}

export async function requestHandwritingTranscription(params:{imageBase64:string;mimeType:string}):Promise<string>{const res=await fetch('/api/writing/transcribe',sameOriginInit({method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(params)}));if(!res.ok){const err=await res.json().catch(()=>({}));throw new GradingError(String(err.code||'unknown'),String(err.error||'Transcription failed'),err);}return String((await res.json()).text||'');}
