...
export async function fetchInitialData(): Promise<{
  ...
  const fallback = {
    profile: { id: 'user_local', targetBand: 7.5, currentLevel: 6.0, hoursPerWeek: 12, weakSection: 'writing' as const, isOnboarded: false },
    tasks: [] as PlanTask[],
    attempts: [] as MockAttempt[],
    checklist: { weekNumber: 1, weekStart: new Date().toISOString().split('T')[0], mocksDone: 0, mocksTarget: 2, essaysDone: 0, essaysTarget: 4, speakingDone: 0, speakingTarget: 5 },
  };
...
export async function syncDataToServer(payload: SyncDataPayload) {
  try {
    ...
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify({ ...parsed, ...payload }));
  ...
  try {
    ...
    if (payload.attempts?.length) {
      ...
      requests.push(Promise.all(attempts.map(attempt => fetch('/api/data/attempts', { method: 'POST', headers: authHeaders({ 'Content-Type': 'application/json' }), body: JSON.stringify(attempt) })) ).then(() => new Response())));
    }
