...
class AiRateLimitService{
  ...
  private appendLog(r:AiUsageRecord){const logs=this.readLogs();logs.push(r);if(logs.length>5000)logs.splice(0,logs.length-5000);const t=`${USAGE_LOG_FILE}.tmp.${process.pid}.${Date.now()}.${nanoid(4)}`;fs.writeFileSync(t,JSON.stringify(logs),'utf8');fs.renameSync(t,USAGE_LOG_FILE);}
