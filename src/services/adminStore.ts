...
class AdminStore {
  ...
  private readCollection<T>(collection: string): T[] {
    ...
    try {
      if (!fs.existsSync(filePath)) { fs.writeFileSync(filePath, JSON.stringify([], null, 2), 'utf-8'); return []; }
      return JSON.parse(fs.readFileSync(filePath, 'utf-8')) as T[];
    } catch (err) { console.error(`[AdminStore] Error reading collection ${collection}:`, err); return []; }
  }

  private writeCollection<T>(collection: string, items: T[]): void {
    const filePath = this.getFilePath(collection);
    try { fs.writeFileSync(filePath, JSON.stringify(items, null, 2), 'utf-8'); }
    catch (err) { console.error(`[AdminStore] Error writing collection ${collection}:`, err); }
  }

  public listMaterials(section: SectionType, statusFilter?: 'all' | 'published' | 'draft'): any[] {
    const items = this.readCollection<any>(section);
    if (statusFilter && statusFilter !== 'all') return items.filter(item => item.status === statusFilter);
    return items;
  }
  ...
  public saveMaterial(section: SectionType, materialData: any, author: string = 'Admin'): any {
    ...
    if (materialData.id) {
      ...
      if (index >= 0) {
        const updated = { ...items[index], ...materialData, updatedAt: now };
        items[index] = updated;
    ...
    const newItem = { ...materialData, id: newId, section, status: materialData.status || 'published', author: materialData.author || author, createdAt: now, updatedAt: now };
  ...
  public deleteMaterial(section: SectionType, id: string): boolean {
    ...
    if (filtered.length !== initialLen) { this.writeCollection(section, filtered); return true; }
    return false;
  }
  ...
  public saveBundle(bundleData: Partial<FullCdiBundle>): FullCdiBundle {
    ...
    if (bundleData.id) {
      ...
      if (index >= 0) {
        const updated = { ...items[index], ...bundleData, updatedAt: now } as FullCdiBundle;
        items[index] = updated;
    ...
    const newBundle: FullCdiBundle = {
      ...
      createdAt: now,
      updatedAt: now,
      timings: bundleData.timings || { listeningMinutes: 30, readingMinutes: 60, writingMinutes: 60, speakingMinutes: 15 },
  ...
  public deleteBundle(id: string): boolean {
    ...
    if (filtered.length !== initialLen) { this.writeCollection('bundles', filtered); return true; }
    return false;
  }
  ...
  public getResolvedBundle(id: string): { bundle: FullCdiBundle; resolvedMaterials: any } | null {
    ...
    return { bundle, resolvedMaterials: { listening, reading, writing, speaking } };
  }
}
