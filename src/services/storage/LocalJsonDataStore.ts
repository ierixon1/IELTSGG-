...
import { isJsonArray, isJsonObject, readLocalJson } from './localJson';

const DATA_DIR=path.join(process.cwd(),'data');
function assertUserId(userId:string){if(!/^[A-Za-z0-9_-]{1,128}$/.test(userId))throw new Error('Invalid user identifier.');}
const isProfile=(value:unknown)=>value===null||isJsonObject(value);
export class LocalJsonDataStore implements DataStore{
 ...
 // A missing file is the fallback; a file that cannot be read or parsed throws instead of reading as empty (M4).
 private read<T>(userId:string,name:string,fallback:T,isShape:(value:unknown)=>boolean=isJsonArray):T{return readLocalJson<T>(path.join(this.file(userId),name),fallback,isShape);}
 ...
 async getUserProfile(userId:string){return this.read<UserProfile|null>(userId,'profile.json',null,isProfile);}
 ...
 async getDailyQuota(userId:string):Promise<DailyQuota>{const dateStr=new Date().toISOString().slice(0,10);const all=this.read<Record<string,{generationsCount?:number;uploadsCount?:number}>>(userId,'quota.json',{},isJsonObject);const d=all[dateStr]||{};return{dateStr,generationsCount:Number(d.generationsCount||0),uploadsCount:Number(d.uploadsCount||0)};}
 async incrementGenerationCount(userId:string){return this.withLock(`${userId}:quota`,async()=>{const q=await this.getDailyQuota(userId),all=this.read<Record<string,DailyQuota>>(userId,'quota.json',{},isJsonObject);all[q.dateStr]={...q,generationsCount:q.generationsCount+1};this.write(userId,'quota.json',all);return this.getDailyQuota(userId);});}
 async reserveGeneration(userId:string,maxGenerations:number){return this.withLock(`${userId}:quota`,async()=>{const q=await this.getDailyQuota(userId);if(q.generationsCount>=maxGenerations)return null;const all=this.read<Record<string,DailyQuota>>(userId,'quota.json',{},isJsonObject);all[q.dateStr]={...q,generationsCount:q.generationsCount+1};this.write(userId,'quota.json',all);return this.getDailyQuota(userId);});}
 async incrementUploadCount(userId:string){return this.withLock(`${userId}:quota`,async()=>{const q=await this.getDailyQuota(userId),all=this.read<Record<string,DailyQuota>>(userId,'quota.json',{},isJsonObject);all[q.dateStr]={...q,uploadsCount:q.uploadsCount+1};this.write(userId,'quota.json',all);return this.getDailyQuota(userId);});}
