...
export class FirestoreDataStore implements DataStore{
 ...
 async saveUserTasks(userId:string,tasks:PlanTask[]){const c=this.subRef(userId,'tasks');const old=await c.get(),keep=new Set(tasks.map(t=>t.id)),b=this.db.batch();old.docs.filter(d=>!keep.has(d.id)).forEach(d=>b.delete(d.ref));tasks.forEach(t=>b.set(c.doc(t.id),t));await b.commit();}
