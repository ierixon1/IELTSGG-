            ...
            <div className="space-y-4">
              <div className="border-b border-slate-100 pb-3">
                ...
                <div className="flex items-center space-x-2 mt-1">
                  <span className="text-xs text-slate-500">Benchmark Target: Band {material.targetBand || '7.5'}</span>
                  <span className="text-slate-300">•</span>
                  <span className="text-xs text-slate-500">Status: {material.status}</span>
                </div>
              </div>
              ...
              {material.section === 'reading' && (
                <div className="space-y-4">
                  <div className="p-5 bg-white rounded-xl border border-slate-200 shadow-2xs">
                    <h4 className="text-base font-bold text-slate-900 mb-2 font-serif">
                      {material.content.passage.title}
                    </h4>
                    <p className="text-xs leading-relaxed text-slate-700 font-serif whitespace-pre-line">
                  ...
                  <div className="space-y-3">
                    ...
                    {material.content.passage.questions.map((q, i) => (
                      <div key={i} className="p-3 bg-slate-50 rounded-lg border border-slate-200 text-xs space-y-1.5">
                        <div className="font-semibold text-slate-900">
                          {i + 1}. {q.questionText}
                        </div>
                        {q.options && (
                          <div className="grid grid-cols-2 gap-1 text-[11px] text-slate-600 pl-4">
              ...
              {material.section === 'listening' && (
                <div className="space-y-4">
                  {material.content.audioUrl && (
                    <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-xl flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs font-bold text-indigo-900">Audio Track Player</span>
                      </div>
                  ...
                  <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                    <h4 className="text-xs font-bold text-slate-900 mb-1">{material.content.section.title}</h4>
                    <p className="text-xs text-slate-600">{material.content.section.contextDescription}</p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    {material.content.section.questions.map((q, i) => (
                      <div key={i} className="p-3 bg-white rounded-lg border border-slate-200 text-xs space-y-1">
                        <div className="font-semibold text-slate-900">{i + 1}. {q.questionText}</div>
                        <div className="text-[11px] text-emerald-700 font-bold">Answer: {q.correctAnswer}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {bundle && (
            <div className="space-y-4">
              <div className="border-b border-slate-100 pb-3">
                ...
                <h3 className="text-xl font-bold text-slate-900">{bundle.bundle.title}</h3>
                <p className="text-xs text-slate-500 mt-1">{bundle.bundle.description}</p>
              </div>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                <div className="p-3 bg-indigo-50 border border-indigo-200 rounded-xl">
                  <div className="text-[10px] font-bold text-indigo-700 uppercase">Listening</div>
                  <div className="text-xs font-extrabold text-slate-900">{bundle.bundle.timings.listeningMinutes} min</div>
                ...
                <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                  ...
                  <div className="text-xs font-extrabold text-slate-900">{bundle.bundle.timings.readingMinutes} min</div>
                  <div className="text-[10px] text-slate-500 truncate mt-1">
                ...
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl">
                  ...
                  <div className="text-xs font-extrabold text-slate-900">{bundle.bundle.timings.writingMinutes} min</div>
                  <div className="text-[10px] text-slate-500 truncate mt-1">
                ...
                <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl">
                  <div className="text-[10px] font-bold text-rose-700 uppercase">Speaking</div>
                  <div className="text-xs font-extrabold text-slate-900">{bundle.bundle.timings.speakingMinutes} min</div>
                  <div className="text-[10px] text-slate-500 truncate mt-1">
