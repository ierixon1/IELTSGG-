...
export async function requireAdminAuth(req:AdminRequest,res:Response,next:NextFunction){try{
  ...
  req.adminSessionToken=session.token;req.adminUser={id:session.userId,username:session.username,displayName:session.name,role:session.role};return next();
 }catch(error){
  // A session store that fails is the server's problem, not a bad credential.
  return next(error);
 }}
